...
func ThreeNodes(opts1, opts2, opts3 *TestNodeOpts) (*serf.Serf, *serf.Serf, *serf.Serf, func(), error) {
	s1, s2, cleanup1, err := TwoNodes(opts1, opts2)
	if err != nil {
		return nil, nil, nil, cleanup1, err
	}
	s3, cleanup2, err := TestNode(opts3)
	cleanup := CombineCleanup(cleanup1, cleanup2)
	if err != nil {
...
func ThreeNodesJoined(opts1, opts2, opts3 *TestNodeOpts) (*serf.Serf, *serf.Serf, *serf.Serf, func(), error) {
	s1, s2, s3, cleanup, err := ThreeNodes(opts1, opts2, opts3)
	if err != nil {
		return nil, nil, nil, cleanup, err
	}
	...
	if n != 2 {
		return nil, nil, nil, cleanup, fmt.Errorf("need 2 successfull joins, got: %d", n)
	}
	return s1, s2, s3, cleanup, err
}
...
func ClientServerRPC(opts *TestNodeOpts) (*client.Client, *grpc.Server, func(), error) {
	...
	server, err := PrepareRPCServer(addr, cert, key, s)
	if err != nil {
		return nil, nil, cleanup2, err
	}
	cleanup3 := CombineCleanup(cleanup2, server.Stop)
	client, err := PrepareRPCClient(addr, cert)
	if err != nil {
		return nil, nil, cleanup3, err
	}
	cleanup4 := CombineCleanup(cleanup3, client.Close)
	return client, server, cleanup4, err
}

func ClientServerRPCFromSerf(s *serf.Serf) (*client.Client, *grpc.Server, func(), error) {
	cert, key, cleanup, err := GenerateSelfSignedCert()
	if err != nil {
		return nil, nil, cleanup, err
	}
	...
	cleanup1 := CombineCleanup(cleanup, server.Stop)
	client, err := PrepareRPCClient(addr, cert)
	if err != nil {
		return nil, nil, cleanup1, err
	}
	cleanup2 := CombineCleanup(cleanup1, client.Close)
	return client, server, cleanup2, err
}
